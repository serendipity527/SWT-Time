...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # SWT 配置
        self.use_swt = getattr(configs, 'use_swt', False)  # 是否使用SWT
        ...
        if self.use_swt:
            print(f"[Time-LLM] 使用 SWT-Patch Embedding: 小波基={self.swt_wavelet}, 分解层数={self.swt_level}")
            self.patch_embedding = WaveletPatchEmbedding(
                configs.d_model, self.patch_len, self.stride, configs.dropout,
                wavelet=self.swt_wavelet, swt_level=self.swt_level, 
                use_all_coeffs=self.use_all_coeffs
            )
        else:
            print("[Time-LLM] 使用原始 Patch Embedding")
